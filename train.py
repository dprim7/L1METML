...
def get_callbacks_from_config(
    ...
    # ReduceLROnPlateau
    if config.get("callbacks.reduce_lr_on_plateau.reduce"):
...
def train_dataGenerator_from_config(config: Config):
    ...
    custom_loss = custom_loss_wrapper(
        ...
        use_mae=config.get("loss.use_mae"),
        mae_weight=config.get("loss.mae_weight"),
        baseline_loss=config.get("loss.baseline_loss"),
        add_respcorr=config.get("loss.add_respcorr"),
        respcorr_factor=config.get("loss.respcorr_factor"),
    )
